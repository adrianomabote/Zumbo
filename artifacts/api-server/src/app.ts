...
app.use("/api", (req, res, next) => {
  if (req.path === "/pagar/webhook" || req.path === "/debitopay/webhook" || req.path === "/paysuite/webhook") {
    return express.raw({ type: "application/json", limit: "256kb" })(req, res, next);
  }
  return next();
});
app.use("/api", (req, res, next) => {
  if (req.path === "/pagar/webhook" || req.path === "/debitopay/webhook" || req.path === "/paysuite/webhook") return pagarRouter(req, res, next);
  return next();
});
